...
const App = () => {
  ...
  const fetchOccupiedSlots = async (local, month) => {
    ...
    try {
      const response = await fetch(`http://localhost:3000/ical/${local}/horarios?timeMin=${timeMin}&timeMax=${timeMax}`);
  ...
  const handleNextCalendarStep = () => {
    if (resumo.evento && resumo.evento.length > 0) {
      ...
      alert("É obrigatório agendar pelo menos um Evento Principal antes de continuar.");
    }
  };
      ...
      {currentStep !== 'select_local' && (
        <>
                    ...
                    if (etapaOrdenada === 'evento' && dados && dados.length > 0) {
                          ...
                          <ul>
                            {dados.map((eventoItem, idx) => (
                              <li key={idx} className="mb-1 flex items-center justify-between ml-4">
                                <div>
                                  {eventoItem.date?.toLocaleDateString()} de {eventoItem.start} até {eventoItem.end}
                                </div>
                                <button
                                  onClick={() => {
                                    setResumo(prevResumo => {
                      ...
                      return (
                        <li key={etapaOrdenada} className="mb-1 flex items-center justify-between">
                          <div>
                            <strong>{displayEtapa}:</strong> {dados.date?.toLocaleDateString()} de {dados.start} até {dados.end}
                          </div>
                          <button
          ...
          {currentStep === 'pessoa_fisica_form' && (
            <PessoaFisicaForm onNext={handlePessoaFormNext} onBack={handlePessoaFormBack} />
          )}

          {currentStep === 'pessoa_juridica_form' && (
            <PessoaJuridicaForm onNext={handlePessoaFormNext} onBack={handlePessoaFormBack} />
          )}

          {currentStep === 'proposta_form' && (
            <Proposta onNext={handlePropostaFormNext} onBack={handlePropostaBack} />
          )}
          ...
          {currentStep === 'resumo_proposta_form' && (
            <ResumoProposta 
              ...
              onConfirm={() => setCurrentStep('acessibilidade_form')}
              onBack={() => setCurrentStep('dados_da_proposta_form')} 
            />
          )}
          ...
          {currentStep === 'acessibilidade_form' && (
            <AcessComu 
              ...
              onBack={() => setCurrentStep('resumo_proposta_form')}
            />
          )}
          ...
          {currentStep === 'resumo_total' && (
            <ResumoTotal
              ...
              onFinalizar={() => {
                console.log('Inscrição finalizada com todos os dados:', dadosCompletos);
                alert("Inscrição finalizada com sucesso! Verifique o console para os dados.");
              }}
