...
app.post('/api/evaluators', requireAdminAuth, async (req, res) => {
  ...
  try {
    ...
    for (const evaluator of evaluators) {
      ...
      const evaluatorPassword = process.env.EVALUATOR_PASSWORD || 'avalia.dac.2026';
      await query(
...
app.get("/api/gerar-termo/:id", async (req, res) => {
  try {
    ...
    const numProp = "____"; // ✅ Alterado para ficar em branco conforme solicitado
    ...
    addClausula("CLÁUSULA SÉTIMA, DAS SANÇÕES ADMINISTRATIVAS", 
      "A prática de ilícitos, as execuções deficientes, irregulares ou inadequadas dos serviços objeto deste termo, o descumprimento de prazos e condições estabelecidas, faculta à AUTORIZADORA, nos termos da Lei, previsto no Item: das Penalidades, a aplicação das seguintes penalidades, independente da que se refere o parágrafo primeiro da cláusula segunda deste termo:\nI - advertência;\nII - multa de 50,00% sobre o valor global da taxa máxima de ocupação do respectivo espaço cultural e tipo de evento, no caso da rescisão por inexecução ou eventual pedido de rescisão sem justo e formal motivo;\nIII - suspensão temporária de autorização de uso de espaço cultural da AUTORIZADORA, pelo prazo de até 02 (dois) anos;\nIV - declaração de inidoneidade para utilização de outros espaços culturais da administração pública federal.",
    ...
    addClausula("CLÁUSULA OITAVA, DAS DISPOSIÇÕES GERAIS", 
...
app.use((req, res) => {
  ...
  const mainIndexPath = path.join(__dirname, 'public', 'index.html');
  if (fs.existsSync(mainIndexPath)) {
    res.sendFile(mainIndexPath);
  } else {
    // Fallback para o dist raiz se o backend/public falhar
