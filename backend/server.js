...
app.post("/api/config", async (req, res) => {
  try {
    ...
    // ✅ EXTRAÇÃO ROBUSTA: Suporta link inteiro do Forms ou Sheets
    const extractId = (url) => {
      if (!url) return "";
      // Suporta /d/ID/ ou /d/e/ID/
    ...
    if (updatedConfig.formsLink) {
      updatedConfig.formsLink = extractId(updatedConfig.formsLink);
    }
    ...
    try {
      ...
      console.error("❌ Erro ao salvar no banco de dados (usando fallback local):", dbError.message);
    }

    // 4. Também salva no arquivo local (Sempre salva como backup/fallback)
    try {
      fs.writeFileSync("config.json", JSON.stringify(updatedConfig, null, 2));
      console.log("✅ Configurações salvas no arquivo local.");
    } catch (fsError) {
      console.error("❌ Erro ao salvar arquivo local:", fsError.message);
    }
    
    res.json({ success: true, ...updatedConfig, savedToDB });
