...
const Admin = ({ viewOnly = false }) => {
  ...
  // ✅ ACESSO CONTROLADO: Verifica se o avaliador está logado
...
// ✅ FUNÇÃO DE LOGIN DO AVALIADOR (handleViewerLogin)
  ...
  <div className={`space-y-1 text-sm ${u.conflictColor ? u.conflictColor.split(' ')[1] : ''}`}>
    {u.ensaio_inicio && <div className="whitespace-nowrap"><strong>Ensaio:</strong>{` ${new Date(u.ensaio_inicio.includes('Z') ? u.ensaio_inicio : u.ensaio_inicio + '-03:00').toLocaleDateString("pt-BR", { day: '2-digit', month: '2-digit', timeZone: 'America/Sao_Paulo' })}, ${new Date(u.ensaio_inicio.includes('Z') ? u.ensaio_inicio : u.ensaio_inicio + '-03:00').toLocaleTimeString("pt-BR", { hour: '2-digit', minute: '2-digit', timeZone: 'America/Sao_Paulo' })} - ${new Date(u.ensaio_fim.includes('Z') ? u.ensaio_fim : u.ensaio_fim + '-03:00').toLocaleTimeString("pt-BR", { hour: '2-digit', minute: '2-digit', timeZone: 'America/Sao_Paulo' })}`}</div>}
    {u.montagem_inicio && <div className="whitespace-nowrap"><strong>Montagem:</strong>{` ${new Date(u.montagem_inicio.includes('Z') ? u.montagem_inicio : u.montagem_inicio + '-03:00').toLocaleDateString("pt-BR", { day: '2-digit', month: '2-digit', timeZone: 'America/Sao_Paulo' })}, ${new Date(u.montagem_inicio.includes('Z') ? u.montagem_inicio : u.montagem_inicio + '-03:00').toLocaleTimeString("pt-BR", { hour: '2-digit', minute: '2-digit', timeZone: 'America/Sao_Paulo' })} - ${new Date(u.montagem_fim.includes('Z') ? u.montagem_fim : u.montagem_fim + '-03:00').toLocaleTimeString("pt-BR", { hour: '2-digit', minute: '2-digit', timeZone: 'America/Sao_Paulo' })}`}</div>}
    {u.eventos_json && JSON.parse(u.eventos_json).map((ev, i) => ( <div key={`evento-${i}`} className="whitespace-nowrap"><strong>Evento {i + 1}:</strong>{` ${new Date(ev.inicio.includes('Z') ? ev.inicio : ev.inicio + '-03:00').toLocaleDateString("pt-BR", { day: '2-digit', month: '2-digit', timeZone: 'America/Sao_Paulo' })}, ${new Date(ev.inicio.includes('Z') ? ev.inicio : ev.inicio + '-03:00').toLocaleTimeString("pt-BR", { hour: '2-digit', minute: '2-digit', timeZone: 'America/Sao_Paulo' })} - ${new Date(ev.fim.includes('Z') ? ev.fim : ev.fim + '-03:00').toLocaleTimeString("pt-BR", { hour: '2-digit', minute: '2-digit', timeZone: 'America/Sao_Paulo' })}`}</div>))}
    {u.desmontagem_inicio && <div className="whitespace-nowrap"><strong>Desmontagem:</strong>{` ${new Date(u.desmontagem_inicio.includes('Z') ? u.desmontagem_inicio : u.desmontagem_inicio + '-03:00').toLocaleDateString("pt-BR", { day: '2-digit', month: '2-digit', timeZone: 'America/Sao_Paulo' })}, ${new Date(u.desmontagem_inicio.includes('Z') ? u.desmontagem_inicio : u.desmontagem_inicio + '-03:00').toLocaleTimeString("pt-BR", { hour: '2-digit', minute: '2-digit', timeZone: 'America/Sao_Paulo' })} - ${new Date(u.desmontagem_fim.includes('Z') ? u.desmontagem_fim : u.desmontagem_fim + '-03:00').toLocaleTimeString("pt-BR", { hour: '2-digit', minute: '2-digit', timeZone: 'America/Sao_Paulo' })}`}</div>}
  </div>
</td>
