...
const confirmStage = (etapa) => {
  ...
  try {
    // 1️⃣ Coletar todos os eventIds que precisam ser removidos (antes de alterar o estado)
    const eventosParaRemover = pendingRemovals.map((removal) => {
      if (removal.etapa === "evento") {
        return resumo.evento[removal.idx]
          ? { nome: `evento ${removal.idx + 1}`, eventId: resumo.evento[removal.idx].eventId }
          : null;
      } else if (resumo[removal.etapa]) {
        return { nome: removal.etapa, eventId: resumo[removal.etapa].eventId };
      }
      return null;
    }).filter(Boolean);

    console.log("🗑️ Removendo eventos:", eventosParaRemover);

    // 2️⃣ Fazer as requisições de remoção em paralelo
    await Promise.all(eventosParaRemover.map(async ({ nome, eventId }) => {
      if (!eventId) {
        console.warn(`⚠️ Evento "${nome}" não possui eventId (não foi salvo no Google Calendar).`);
        return;
      }

      console.log(`➡️ Enviando DELETE para evento "${nome}" com ID: ${eventId}`);
      const res = await fetch(`http://localhost:4000/api/cancel-events/${localSelecionado}`, {
        method: "DELETE",
      ...
      console.log(`✅ Resultado da exclusão de "${nome}":`, result);
    }));

    // 3️⃣ Atualizar estado apenas após todos os deletes concluírem
    setResumo((prevResumo) => {
      const novoResumo = { ...prevResumo };

      pendingRemovals.forEach((removal) => {
        if (removal.etapa === "evento") {
          novoResumo.evento = novoResumo.evento.filter((_, i) => i !== removal.idx);
        } else {
          delete novoResumo[removal.etapa];
        }
      });
...
if (res.status === 404) {
 ...
 const handleSendEmail = async () => {
  try {
    ...
    // 🔎 LOG PARA DEBUG
    console.log("📤 Construindo etapas para enviar ao backend...");
    ...
    if (resumo.ensaio) {
      ...
      console.log("➕ Ensaio adicionado:", etapas[etapas.length - 1]);
    }
    ...
    if (resumo.montagem) {
      ...
      console.log("➕ Montagem adicionada:", etapas[etapas.length - 1]);
    }

    if (resumo.evento.length > 0) {
      resumo.evento.forEach((ev, i) => {
        etapas.push({
          nome: "evento",
          inicio: `${ev.date.split("T")[0]}T${ev.start}:00`,
          fim: `${ev.date.split("T")[0]}T${ev.end}:00`,
        });
        console.log(`➕ Evento ${i + 1} adicionado:`, etapas[etapas.length - 1]);
      });
    }

    // ✅ Agora garantimos que desmontagem será enviada se existir no resumo
    if (resumo.desmontagem) {
      ...
      console.log("➕ Desmontagem adicionada:", etapas[etapas.length - 1]);
    }

    // Se não houver etapas, não envia nada
    if (etapas.length === 0) {
      setAlertMessage("⚠️ Nenhuma etapa foi selecionada.");
      return;
    }

    // 🔎 LOG FINAL
    console.log("📤 Etapas finais enviadas para backend:", etapas);
    ...
    if (result.success) {
      ...
      // 🔧 Corrigido: atribuição de eventId usando comparação robusta
      setResumo((prevResumo) => {
        const novoResumo = { ...prevResumo };

        result.eventosCriados.forEach((eventoCriado) => {
          const summary = eventoCriado.summary.toLowerCase();

          if (summary.includes("ensaio") && novoResumo.ensaio) {
            novoResumo.ensaio.eventId = eventoCriado.id;
          }
          if (summary.includes("montagem") && novoResumo.montagem) {
            novoResumo.montagem.eventId = eventoCriado.id;
          }
          if (summary.includes("desmontagem") && novoResumo.desmontagem) {
            novoResumo.desmontagem.eventId = eventoCriado.id;
          }
          if (summary.includes("evento")) {
            // Atribui ao primeiro evento sem eventId
            const idx = novoResumo.evento.findIndex((ev) => !ev.eventId);
            if (idx !== -1) novoResumo.evento[idx].eventId = eventoCriado.id;
          }
        });
      ...
      setAlertMessage("✅ Primeira etapa concluída! Eventos salvos no Google Calendar e e-mails enviados pelo servidor.");
    } else {
