              ...
              <tbody>
                {unificados.map((u, idx) => (
                  <tr key={idx}>
                    ...
                    <td className={`border px-2 py-1 ${u.validado ? "text-green-600" : "text-red-600"}`}>
                      {u.validado ? "✅" : "❌"}
                    </td>
                    <td className="border px-2 py-1">
                        ...
                        {u.anexos && u.anexos.length > 0 ? (
                          u.anexos.map((arq, i) => (
                            <li key={i} className="flex items-center gap-2">
                              <a
                                href={`https://drive.google.com/file/d/${arq.fileId}/preview`}
                                target="_blank"
                                rel="noopener noreferrer"
                                className="text-blue-600 hover:underline"
                              >
                                👁️ Visualizar Arquivo {i + 1}
                              </a>
                              <a
					  ...
					  <button
  onClick={() => window.open(`http://localhost:4000/api/download-zip/${u.id}`, "_blank")}
  className="mt-2 px-3 py-1 bg-green-600 text-white rounded hover:bg-green-700 text-sm"
>
  ⬇️ Baixar todos em ZIP
</button>
